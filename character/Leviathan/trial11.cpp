...
int main() {
    ...
    for (int i = 0; i <= stackCount; ++i) {
        float stackAngle = glm::pi<float>() * static_cast<float>(i) / static_cast<float>(stackCount);
        float y = radius * std::cos(stackAngle);
        float xy = radius * std::sin(stackAngle);

        for (int j = 0; j <= sectorCount; ++j) {
            float sectorAngle = 2.0f * glm::pi<float>() * static_cast<float>(j) / static_cast<float>(sectorCount);
